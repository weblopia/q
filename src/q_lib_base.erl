...
%%% @doc
%%%
%%% http:some wikipedia page here comming soon
%%%
%%% @end
%%% Created : 09. Feb 2017 8:51 PM
%%%-------------------------------------------------------------------
-module(q_lib_base).
-author("madalin").
...
%% This approach allow baseq encoding to achieve almost same output size as input data
%% when input data is composed mainly by english texts.
%%
%% On best case scenario we can achieve the amasing 1:1 input/output length ratio.
%% For bytes outside alpha-numeric ASCII characters BaseQ encoding use 2 bytes.
%% In worst case scenarios we can achieve 1:2 input/output length ratio.
%%
%% ZQXJ where chose based on relative frequencies of letters in the dictionaries of 15 languages that use all or parts of
%% the latin alphabet.
%% We also used information from Robert Lewand's Cryptological Mathematics for english language.
%% Acording to Robert Lewand the less frequent letter in english language are:
%%
%% z	0.074%
%% q	0.095%
%% x	0.150%
%% j	0.153%
%%
%% We also took into consideration that in most languages upper case letters are always used less than lower case ones.
%% That's why we decided to use ZQXJ letters as markers for double byte encoded bytes (bytes outside alpha-numeric characters).
%%
...
encode_basel_byte(Byte) when Byte < 48 -> encode_basel_byte_holle(Byte);
%% Ignore 0-9 holle
encode_basel_byte(Byte) when Byte > 57, Byte < 65 -> encode_basel_byte_holle(Byte - 10);
%% Ignore 0-9, A-I holle
encode_basel_byte(Byte) when Byte > 57, Byte < 65 -> encode_basel_byte_holle(Byte - 10);
%% Ignore 0-9, A-I, K-P holle
encode_basel_byte(81) -> encode_basel_byte_holle(55);
%% Ignore 0-9, A-I, K-P, R-W holle
encode_basel_byte(Byte) when Byte > 89, Byte < 97 -> encode_basel_byte_holle(Byte - 34);
%% Ignore 0-9, A-I, K-P, R-W, Y holle
encode_basel_byte(113) -> encode_basel_byte_holle(63);
%% Ignore 0-9, A-I, K-P, R-W, Y, a-y holle
encode_basel_byte(Byte) when Byte > 121 -> encode_basel_byte_holle(Byte - 58).

%% Encode byte with holle
encode_basel_byte_holle(Byte) ->
...
%% Restore 0-9 AND A-P holle (Q ASCII letter)
decode_basel_byte_hole(55) -> 81;
%% Restore 0-9, A-P AND R-Y holle
decode_basel_byte_hole(Byte) when Byte > 55, Byte < 63 -> Byte + 34;
%% Restore 0-9, A-P, R-Y AND a-p holle (q ASCII letter)
decode_basel_byte_hole(63) -> 113;
%% Restore 0-9, A-P, R-Y, a-p AND r-y holle
...
test() ->
  ...
  %% BaseQ encode the table
  EncodedBaseQTable = encode_basel(Table),
  io:format("~n~nEncoded BaseQ table (size ~p): ~n~p", [erlang:size(EncodedBaseQTable), EncodedBaseQTable]),

  %% BaseQ decode the table
  DecodedBaseQTable = decode_basel(EncodedBaseQTable),
  io:format("~n~nDecoded BaseQ table (size ~p): ~n~p", [erlang:size(DecodedBaseQTable), DecodedBaseQTable]),
  ...
  %% Benchmark
  q_lib_benchmark:function(qbase_lib_base, encode_basel, [Table], 100),
  q_lib_benchmark:function(qbase_lib_base, encode_base32, [Table], 100),

  %% Check
  Table = DecodedBaseQTable,
