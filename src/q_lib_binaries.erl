...
%%%
%%% @end
%%% Created : 07. Feb 2017 10:39 PM
%%%-------------------------------------------------------------------
-module(q_lib_binaries).
-author("madalin").
...
-define(QWE_LIB_BINARIES_HEX_UPPER_BYTE(Byte), (hex_upper_byte(Byte))/binary).
-define(QWE_LIB_BINARIES_HEX_LOWER_BYTE(Byte), (hex_lower_byte(Byte))/binary).
...
convert_to_hex_upper(<<A:8, B:8, C:8, D:8, E:8, F:8, G:8, H:8, Rest/binary>>, Acumulator) ->
  ...
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(A),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(B),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(C),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(D),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(E),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(F),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(G),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(H)>>);
convert_to_hex_upper(<<X:8, Rest/binary>>, Acumulator) -> convert_to_hex_upper(Rest, <<Acumulator/binary, ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(X)>>).
...
convert_to_hex_lower(<<A:8, B:8, C:8, D:8, E:8, F:8, G:8, H:8, Rest/binary>>, Acumulator) ->
  ...
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(G),
  ?QWE_LIB_BINARIES_HEX_UPPER_BYTE(H)>>);
convert_to_hex_lower(<<X:8, Rest/binary>>, Acumulator) -> convert_to_hex_lower(Rest, <<Acumulator/binary, ?QWE_LIB_BINARIES_HEX_LOWER_BYTE(X)>>).
